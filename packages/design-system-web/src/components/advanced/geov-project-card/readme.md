# geov-project-card



<!-- Auto Generated Below -->


## Properties

| Property         | Attribute          | Description | Type     | Default     |
| ---------------- | ------------------ | ----------- | -------- | ----------- |
| `geovStyle`      | `geov-style`       |             | `string` | `''`        |
| `projectDesc`    | `project-desc`     |             | `string` | `undefined` |
| `projectPageUrl` | `project-page-url` |             | `string` | `undefined` |
| `projectTitle`   | `project-title`    |             | `string` | `undefined` |


## Dependencies

### Depends on

- [geov-card](../geov-card)
- [geov-row](../../grid/geov-row)
- [geov-button](../../basic/geov-button)
- [geov-column](../../grid/geov-column)
- [geov-heading](../../basic/geov-heading)
- [geov-text](../../basic/geov-text)

### Graph
```mermaid
graph TD;
  geov-project-card --> geov-card
  geov-project-card --> geov-row
  geov-project-card --> geov-button
  geov-project-card --> geov-column
  geov-project-card --> geov-heading
  geov-project-card --> geov-text
  geov-card --> geov-column
  geov-button --> geov-row
  geov-button --> geov-icon
  geov-button --> geov-text
  style geov-project-card fill:#f9f,stroke:#333,stroke-width:4px
```

----------------------------------------------

*Built with [StencilJS](https://stenciljs.com/)*
